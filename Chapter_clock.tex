...
\bigskip
La fréquence du signal de sortie est :\begin{LARGE} $F_{out}=\frac{N}{M}.F_{in}$\end{LARGE}
...
Le schéma détaillé du circuit UCS est donné à la figure \ref{fig:UCS_Schema}. En plus des 5 sources de fréquence principales décrites précédemment, une $6^{eme}$ source est visible: il s'agit de la sortie du \it prescaler\rm, elle est notée DCOCLKDIV. De plus, 2 autres signaux d'horloge sont visibles, en plus de ACLK, MCLK et SMCLK. Ce sont ACLK/n, qui est dérivée de ACLK par une division de fréquence, et MODCLK, qui sert uniquement pour le convertisseur AD. Finalement, les horloges ACLK, MCLK et SMCLK peuvent subir une prédivision de fréquence par 1, 2, 4, 8, 12, 16, 32.
...
Ils sont nommés UCSCTLx et sont au nombre de 10. Toutefois, les plus couramment utilisés sont :
\begin{itemize}[label=\textbullet,font=\small]
\item UCSCTL1: contient DCORSEL;
\item UCSCTL2 et UCSCTL3 : configuration du multiplieur de fréquence FLL; 
\item UCSCTL4: connections de ACLK, MCLK et SMCLK;
\item UCSCTL5: facteurs de prédivision de ACLK, MCLK et SMCLK;
\item .
\end{itemize}
